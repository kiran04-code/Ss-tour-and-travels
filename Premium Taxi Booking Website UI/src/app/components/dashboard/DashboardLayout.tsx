...
export function DashboardLayout({ title, eyebrow = "OWNER PORTAL", children }: { title: string; eyebrow?: string; children: ReactNode }) {
  return <div className="min-h-screen bg-[#F7F9FC] text-[#071D49]">
    ...
    <div className="mx-auto max-w-[1320px] px-5 py-8 lg:px-8"><div className="mb-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="text-[10px] font-bold tracking-[.2em] text-[#9a7100]">{eyebrow}</p><h1 className="mt-2 text-3xl font-extrabold tracking-tight sm:text-4xl">{title}</h1></div><nav className="flex flex-wrap gap-2 text-xs font-bold"><a href="/dashboard" className="inline-flex items-center gap-1.5 border border-[#DDE4EF] bg-white px-3 py-2 hover:border-[#F9B900]"><LayoutDashboard size={14} /> Overview</a><a href="/dashboard/cars/new" className="inline-flex items-center gap-1.5 bg-[#F9B900] px-3 py-2 text-[#071D49]"><Plus size={14} /> Add Car</a><a href="/admin" className="inline-flex items-center gap-1.5 border border-[#DDE4EF] bg-white px-3 py-2 hover:border-[#F9B900]"><ShieldCheck size={14} /> Admin</a><a href="/cars" className="inline-flex items-center gap-1.5 border border-[#DDE4EF] bg-white px-3 py-2 hover:border-[#F9B900]"><ListChecks size={14} /> Browse Cars</a></nav></div>{children}<GalleryManager /></div>
  </div>;
}
